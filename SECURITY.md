...
- Operation journal format v2 durably records the immutable plan, selected Agent/snapshot, expanded scope, backup phases, and per-mutation progress. A recognized non-terminal journal blocks mutation only for its owning Agent; an unknown committed journal blocks mutation globally. Recovery prompts may be dismissed for read-only browsing, but mutation commands enforce the block again. Strictly recognized pre-v2 status-only journals are removed as obsolete metadata. Recovery first rescans the owning Agent and never infers success from journal state alone.
