...
| OpenCode | Sessions, offline | Official `opencode session delete` with `OPENCODE_DB` | Recognized official schema, matching durable database/WAL revision, and no writer | Official `export` / `import` inside encrypted backup staging | Exact CLI argument test, command-failure test, descendant planning, durable-revision change rejection with shared-memory reader-lock exclusion, import preflight, and compensation tests |
